---
title: "Nike NLP Analysis"
subtitle: "Text Analytics for Strategic Brand Intelligence"
...
```{r no-brands}
# version without brand names — for sentiment + LDA so brand names
...
We stick with the original `word` column since sentiment lexicons match on full words, not stems.
...
## 1f: Document-Term Matrix

```{r dtm}
# needed for LDA — rows = docs, columns = words, values = counts
...
## 2c: Comparing Sentiment Methods
...
## 2d: Top Positive & Negative Words
...
LDA finds hidden themes — each doc is a mix of topics, each topic is a mix of words.

## 3a: Nike-Specific LDA (k=4)
...
## 5a: Pairwise Correlations (Nike)

```{r pairwise}
# only words appearing 20+ times
...
## 5c: Correlation Network

```{r cor-network, fig.width=12, fig.height=8}
nike_word_cors %>%
...
```{r bigram-network, fig.width=12, fig.height=8}
...
**Q2 — How do people feel about each brand's pricing?**
...
## 7a: Word Frequency by Brand

```{r freq-corr, fig.width=12, fig.height=6}
frequency <- bind_rows(
